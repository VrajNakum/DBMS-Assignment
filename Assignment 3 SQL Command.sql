...
Insert into emp values  
(  'Rahul', 'IT',1000000,8000 ,5,'Zori', null)
...
Select Edepartment, COUNT (Ename) as EmpName
from emp
group by Edepartment
--order by 2 desc
...
Select EDepartment , MAX (ESalary) as MaxEmp , AVG (ESalary) as avgEmp
from emp 
Group by Edepartment  
--order by 1 desc
...
Select Edepartment , min (Esalary) as MinESalary
from emp
group by Edepartment
...
-- than 100,000
 ...
 update emp
 set ESalary = 11000000
 where EID = 1


 Select Edepartment , sum(Esalary) as SumESalary
 from emp 
 Group by Edepartment
 ...
 Select Edepartment , Avg (Esalary) as avgEsalary
 from emp
 group by Edepartment
 ...
 Select Edepartment , Count (Ename ) as CountEname
 from emp 
 group by Edepartment 
 ...
 Select   Edepartment , max (ename) as maxEname  , max ( ESalary) as maxEsalary
 from emp
 group by Edepartment 
 order by 2 desc
 ...
 -- 11. 11.Write a query to list all employees sorted by Department alphabetically, and
-- then by Ename alphabetically.

Select * from emp 
order by Edepartment , Ename

Select Edepartment , Ename 
from emp
order by Edepartment , Ename
